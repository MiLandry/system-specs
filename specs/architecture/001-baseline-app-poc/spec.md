...
- **FR-008**: The frontend workspace MUST provide **`bun run clean`** (remove `dist/`, `dist-ssr/`, `src/generated/` OpenAPI output, and Vite/TypeScript caches under `node_modules` without deleting packages) and **`bun run nuke`** (`clean`, then remove `node_modules/` and run `bun install`, keeping `bun.lock` and `.env`) via `scripts/clean.ts` and `scripts/nuke.ts` as documented in [`quickstart.md`](quickstart.md). Generated API types are recreated by `prebuild:app` on the next `build:app`.
